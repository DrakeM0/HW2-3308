...
function begin_play(){
	...
	if(game_started()){
		alert("Game has started hit reset and try again");
	}
	else if(isEmpty(p1.value) || isEmpty(p2.value)){
		alert("you need two to tango");
	}else
	{
	// game start and no changing names
	//also set players to xsandos
...
function play(){
	...
	if(!game_started()){//check game state
	...
	if(check == false){
		alert("cant go their. your ontop a peace or youve inputed wrong");
		return;
	}


	if(this.turn == 1){//p1s turn
